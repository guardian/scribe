      ...
      InsertListCommand.prototype.execute = function (value) {
        function splitList(listItemElements) {
          if (listItemElements.length > 0) {
            var newListNode = scribe.el.ownerDocument.createElement(listNode.nodeName);
        ...
        if (this.queryState()) {
          ...
          scribe.transactionManager.run(function () {
            if (listItemElement) {
              ...
              var documentFragment = scribe.el.ownerDocument.createDocumentFragment();
              selectedListItemElements.forEach(function (listItemElement) {
                var pElement = scribe.el.ownerDocument.createElement('p');
